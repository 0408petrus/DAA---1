...
int getlistLength(barang** head) {
    int count { 0 };
    if (*head == NULL) {
        return count;
    }
    barang* data_todisplay = *head;
...
barang* getNodeatIndex(barang** head, int index) {
    int i{ 0 };
    barang* current = *head;
    while (current != NULL) {
        if (i == index) {
...
barang* createNodeCopy(barang* source) {
    barang* copy = new barang;
...
void copypasteNode(barang* copy, barang* paste) {
	strcpy((*paste).nama,(*copy).nama);
    (*paste).id = (*copy).id;
    (*paste).harga = (*copy).harga;
}

void swapNode(barang* m, barang* n) {
    barang* temp = createNodeCopy(m);
    copypasteNode(n, m);
